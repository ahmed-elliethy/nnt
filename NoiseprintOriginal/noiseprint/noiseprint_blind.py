...
def noiseprint_blind_file(filename, model_name='net'):
    ...
    res, mapp, valid, range0, range1, imgsize, other = noiseprint_blind(img, QF, model_name=model_name)
    return res, QF, mapp, valid, range0, range1, imgsize, other
...
def noiseprint_blind_print(res, img):
    assert(img.shape==res.shape)
    return noiseprint_blind_post(res, img)
...
def noiseprint_blind_post(res, img):
    ...
    if np.sum(valid)<50:
        ...
        return res, None, valid, range0, range1, imgsize, dict()
    
    mapp, other = EMgu_img(spam, valid, extFeat = range(32), seed = 0, maxIter = 100, replicates = 10, outliersNlogl = 42)
    
    return res, mapp, valid, range0, range1, imgsize, other
